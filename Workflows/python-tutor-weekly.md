...
**When to run:** Sundays (manual or scheduled). Jon says "weekly python review" or this is invoked by the weekly routine.
...
- `Resources/python-learning-plan.md` — current weekly goal and where Jon expected to be by today
...
- **Drift from plan** — did Jon finish the weekly goal (typically: 1 MOOC Part)? If not, by how much?
...
- Update Jon's **current position** if he completed the previous Part
...
### 6. Brief sign-off to Jon

Show Jon the summary section you just wrote (paste it back in chat) and confirm next week's plan with him. Use `AskUserQuestion` if any aspect of next week's focus is genuinely ambiguous (e.g., "skip Part 6 for now and prioritize a mini-project?"). Otherwise, keep it short.

---

## Anti-patterns

- Don't grade the week harshly if Jon missed a day or two — life happens. Acknowledge, adjust, move on.
