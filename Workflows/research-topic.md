...
**When to run:** the user says "run the research workflow on \<topic\>" or "research \<topic\> for me".
...
Skip this step if the topic and scope are unambiguous from how the user phrased it.
...
**Length target: ~40 lines, ~300 words. Half a page.** the user skims, he doesn't read.
...
- 3 bullets max — the things the user needs to decide or act on
...
- `## Background / Why it matters` — only if the user can't understand the Findings without it. 3 bullets max.
- `## Open Questions / Caveats` — only if there are real unknowns or contested claims that would change the user's action. 3 bullets max.
...
Tell the user the report path (e.g., `Output/research/prompt-caching-for-anthropic-api.md`) and a 1-sentence summary of what's inside. Don't paste the full report into chat — he can open the file.
